...
function findPinkElephantsByTimestreet(songs) {
  const pink = songs.find(function(song){
    if (song.title === "Pink Elephants" && song.artist === "Timestreet") {
      return true;
    }
  });
    return pink;
...
function findFirstSongUnderThreeMinutes(songs) {
  const firstSong = songs.find(function(song){
...
function findFirstTitleTrack(songs) {
  const sameTitleAndAlbum = songs.find(function(song){
