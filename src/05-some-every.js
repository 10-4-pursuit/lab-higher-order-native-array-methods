...
function allSongsAreOverTwoMinutes(songs) {
  // Using the every method to check if every song has a runtime over two minutes
  return songs.every((song) => song.runtime > 120);
}
...
function anySongIsOverFourMinutes(songs) {
  // Using the some method to check if any song has a runtime over four minutes
  return songs.some((song) => song.runtime > 240);
}
...
function anySongIsByPeanut(songs) {
  // Using the some method to check if any song is by the artist "Peanut"
