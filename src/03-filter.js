...
return songs.filter(object => {
  if (object.artist === 'Saib') {
    return object
  }
})
}
...
function getSongsOverThreeMinutes(songs) {
  return songs.filter(object => object.runtimeInSeconds > 180)
 }
...
function getTitleTracks(songs) {
  return songs.filter(element => element.title === element.album) 
